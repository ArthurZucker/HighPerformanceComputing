...
struct csr_matrix_t *load_mm(FILE *f)
{
	...
	// displs[0]=0;
	// for (int i = 0; i < nbp; i++) {
	// 				scounts[i] = (n/nbp)+ 2 + (n%nbp)*(i==nbp-1); //combien d'infos j'envoie
	// 				displs[i]  = i*(n/nbp);//pointeur sur où écrire
	// }
	//
	// fprintf(stderr,"%d \n",n%nbp*(rang==nbp-1));
	// MPI_Scatterv(Ap,scounts,displs,MPI_INT,&Ap[u1],(n%nbp)*(rang==nbp-1)+2+n/nbp,MPI_INT,0,MPI_COMM_WORLD);
	// fprintf(stderr,"%d : Ap \n",rang);
...
void cg_solve(const struct csr_matrix_t *A, const double *b, double *x, const double epsilon, double *scratch)
{
	...
	for (int i = 0; i < nbp; i++) {
		int u = i*n/nbp;
		displs[i] = u;
		rcounts[i] = n/nbp + (n%nbp)*(i==nbp-1 && n%nbp!=0);
	}
...
int main(int argc, char **argv)
{
	...
	if (safety_check)
	{
		MPI_Allgatherv(&x[rang*n/nbp],n/nbp + (n%nbp)*(rang==nbp-1 && n%nbp!=0), MPI_DOUBLE, x, rcounts, displs, MPI_DOUBLE, MPI_COMM_WORLD);
		double *y = scratch;
		sp_gemv(A, x, y);											// y = Ax
