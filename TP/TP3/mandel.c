...
int main(int argc, char *argv[])
{
	...
	if(rang == 0){
		...
		for (size_t i = 1; i < p; i++) {
			MPI_Ssend(&b,1,MPI_INT,i,INDICE,MPI_COMM_WORLD);
			b+=w;
		}
		while (b<h*w) {
			unsigned char *imatemp = (unsigned char *)malloc(w * sizeof(unsigned char));
			int btemp;
			MPI_Recv(&btemp,1,MPI_INT,MPI_ANY_SOURCE,TRAITEMENT,MPI_COMM_WORLD,&status);
			int idtemp = status.MPI_SOURCE;
			MPI_Recv(imatemp,w,MPI_UNSIGNED_CHAR,idtemp,TRAITEMENT,MPI_COMM_WORLD,&status);
			MPI_Ssend(&b,1,MPI_INT,idtemp,INDICE,MPI_COMM_WORLD);
			b+=w;
			for (size_t i = 0; i < w; i++) {
				ima[btemp+i] = imatemp[i];
			}
			free(imatemp);
		}
		for (size_t i = 1; i < p; i++) {
			...
			int idtemp = status.MPI_SOURCE;
			MPI_Recv(imatemp,w,MPI_UNSIGNED_CHAR,idtemp,TRAITEMENT,MPI_COMM_WORLD,&status);
			MPI_Ssend(&b,1,MPI_INT,idtemp,STOP,MPI_COMM_WORLD);
			for (size_t j = 0; j < w; j++) {
				ima[btemp+j] = imatemp[j];
			}
			free(imatemp);
		}
		...
		while(1){
			...
			unsigned char *btemp = (unsigned char *)malloc(w * sizeof(unsigned char));
			y = ymin + b/w*yinc;
			x = xmin;
			for (j = 0; j < w; j++) {
				btemp[j] = xy2color(x, y, prof);
				x += xinc;
			}
			MPI_Ssend(&b,1,MPI_INT,0,TRAITEMENT,MPI_COMM_WORLD);
			MPI_Ssend(btemp,w,MPI_UNSIGNED_CHAR,0,TRAITEMENT,MPI_COMM_WORLD);
			free(btemp);
		}
