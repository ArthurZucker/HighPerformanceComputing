...
int main(int argc, char *argv[])
{
	...
	while (convergence == 0) {
		...
		if (rang%2 == 0) {
			if (rang<p-1) MPI_Sendrecv(&T[((rang+1)*(o/p)*n*m)-n*m]	,n*m, MPI_DOUBLE,rang+1,0,&T[(rang+1)*(o/p)*n*m]		, n*m, MPI_DOUBLE,rang+1, 0,MPI_COMM_WORLD, &status);
			if (rang > 0) MPI_Sendrecv(&T[rang*(o/p)*n*m]						,n*m, MPI_DOUBLE,rang-1,0,&T[((rang)*(o/p)*n*m)-n*m], n*m, MPI_DOUBLE,rang-1, 0,MPI_COMM_WORLD, &status);
		}
		else{
			if (rang<p-1) MPI_Sendrecv(&T[((rang+1)*(o/p)*n*m)-n*m]	,n*m, MPI_DOUBLE,rang+1,0,&T[(rang+1)*(o/p)*n*m]		, n*m, MPI_DOUBLE,rang+1, 0,MPI_COMM_WORLD, &status);
			if (rang > 0) MPI_Sendrecv(&T[rang*(o/p)*n*m]						,n*m, MPI_DOUBLE,rang-1,0,&T[((rang)*(o/p)*n*m)-n*m], n*m, MPI_DOUBLE,rang-1, 0,MPI_COMM_WORLD, &status);
